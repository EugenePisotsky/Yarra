...
impl BoundaryField {
    pub fn for_scene(catalog: &VegetationCatalog, pages: &[VegetationFieldPage]) -> Self {
        ...
        if pages.is_empty() { minimum = Vec2::ZERO; maximum = Vec2::ZERO; }
        Self::bake(catalog, pages, minimum, maximum)
    }

    pub fn bake(catalog: &VegetationCatalog, pages: &[VegetationFieldPage], min: Vec2, max: Vec2) -> Self {
        ...
        let size = ((max + Vec2::splat(MARGIN) - minimum) / step).ceil().as_uvec2().max(UVec2::ONE);
        let mut depth = vec![0.0; size.x as usize * size.y as usize];
        for page in pages {
            let page_min = Vec2::from_array(page.origin_xz);
            let lo = ((page_min - minimum) / step).floor().max(Vec2::ZERO).as_uvec2().min(size);
            let hi = ((page_min + Vec2::splat(page.size) - minimum) / step).ceil().max(Vec2::ZERO).as_uvec2().min(size);
            for field in &page.fields {
                if !catalog.population(field.population).is_some_and(|p| p.key == "short_split_fill") { continue; }
                for y in lo.y..hi.y {
                    for x in lo.x..hi.x {
                        let p = minimum + (Vec2::new(x as f32, y as f32) + Vec2::splat(0.5)) * step;
                        if page.owns(p.to_array()) && field.sample_coverage(page, p.to_array()) >= 0.1
                            && page.surface.sample(page.origin_xz, page.size, p.to_array()).validity >= 0.5 {
    ...
    pub fn gpu_values(&self) -> Vec<f32> {
        let mut data = vec![self.minimum.x, self.minimum.y, self.step, self.size.x as f32,
            self.size.y as f32, 0.0, 0.0, 0.0];
...
fn distance_transform(depth: &mut [f32], size: UVec2, step: f32) {
    ...
    for y in 0..h {
        for x in 0..w {
            let i = y * w + x;
            if x > 0 { depth[i] = depth[i].min(depth[i - 1] + step); }
            if y > 0 {
                depth[i] = depth[i].min(depth[i - w] + step);
                if x > 0 { depth[i] = depth[i].min(depth[i - w - 1] + diagonal); }
                if x + 1 < w { depth[i] = depth[i].min(depth[i - w + 1] + diagonal); }
            }
        }
    }
    for y in (0..h).rev() {
        for x in (0..w).rev() {
            let i = y * w + x;
            if x + 1 < w { depth[i] = depth[i].min(depth[i + 1] + step); }
            if y + 1 < h {
                depth[i] = depth[i].min(depth[i + w] + step);
                if x > 0 { depth[i] = depth[i].min(depth[i + w - 1] + diagonal); }
                if x + 1 < w { depth[i] = depth[i].min(depth[i + w + 1] + diagonal); }
    ...
    for value in depth { *value = (*value - step * 0.5).clamp(0.0, MAX_DEPTH); }
}
