...
use world::{
    AssetId, CellCoord, DEFAULT_CELL_SIZE, PageCodec, PageDomain, PageKey, PagePayload,
...
use world_db::{
    AssetVariantRecord, EncodedPage, PageDependencyRecord, ProjectDocument, RuntimeBuild,
    RuntimeCellRecord, RuntimeManifest, SourceAssetRecord, SourceCellRecord, SourceObjectRecord,
...
pub fn build_runtime(mut project: ProjectDocument) -> Result<RuntimeBuild> {
    ...
    let mut pages = Vec::with_capacity(project.cells.len() * 2);
    ...
    for source_cell in &project.cells {
        ...
        if !source_objects.is_empty() {
            ...
            let instances = source_objects
                .iter()
                .map(|object| StaticObjectInstance {
                    id: object.id,
                    asset: object.asset,
            ...
            for object in source_objects {
                if !depended_assets.insert(object.asset) {
                    continue;
                }
