...
[Precipitation](../crates/atmosphere/src/precipitation.rs) draws rain as instanced camera-facing streaks in three camera-anchored boxes (near, mid, far) that wrap a world-fixed drop lattice. Positions come from hashes plus a CPU-accumulated fall offset, so there is no per-drop state and wind changes never make drops jump. Streaks follow drop velocity relative to the smoothed camera velocity (teleports and origin rebases ignored), keep at least one pixel of width with proportionally reduced alpha, fade near the lens and with weather fog, and use a soft test against scene depth. The pass runs between `EarlyPostProcess` and `PostProcess`: after Temporal reconstruction (no ghosting, output resolution) or on the resolved MSAA image, and before bloom and tone mapping. Wetness from `AtmosphereState` reaches every surface through `CloudParams::weather`: the shared PBR adapter darkens albedo and lowers roughness on sky-facing surfaces, and grass darkens its body colour.

[Rain shelter](../crates/atmosphere/src/shelter.rs) is a 256² top-down map (0.5 m texels, 128 m around the camera) of the highest object top above each texel and its coverage. Any placed object shelters the ground below its top, so trees, shrubs and rocks need no tagging: streamed objects carry an `ObjectFootprint` from their largest LOD bounds, and the [engine](../crates/engine/src/weather.rs) rebuilds the map while rain or wetness needs it, after moving 16 m, on origin rebases and (throttled to 0.5 s) when objects stream. Materials bind it at 123 beside the cloud inputs; rain streaks, splash placement, surface wetness and puddles all use the same bilinear lookup. Splashes are spawned on the CPU on resident terrain ahead of the camera (skipping steep and sheltered ground) into a 512-entry ring buffer and drawn as short-lived crowns in the rain pass, hidden behind nearer geometry such as grass. [Puddles](../assets/shaders/weather/ground_wetness.wgsl) form on flat, soaked terrain from world-anchored noise (about 2% of flat ground at first, 10% when soaked) with ripple rings while rain falls; only the terrain shaders call it.
