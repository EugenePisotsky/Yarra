#!/usr/bin/env python3
"""Offline GP-023 counter comparison; no game, GPU work or sudo."""
import hashlib
import json
...
def analyze():
    with zipfile.ZipFile(HERE / 'evidence.zip') as archive, tempfile.TemporaryDirectory() as temp:
        ...
        def mean_instructions(variant, counter):
            matching = [r for r in rows if r['variant'] == variant]
            return sum(int(r['main_counters'][counter]) for r in matching) / len(matching)
        means = {v: {k: mean_instructions(v, k) for k in
                     ('VS ALU Instructions', 'FS ALU Instructions')}
                 for v in ('baseline', 'candidate')}
        changes = {k: 100 * (means['candidate'][k] / means['baseline'][k] - 1)
                   for k in means['baseline']}
        changes['Combined main VS+FS ALU Instructions'] = 100 * (
            sum(means['candidate'].values()) / sum(means['baseline'].values()) - 1)
        return {
            'experiment': 'GP-023', 'changed_shaders': changed,
            'inputs': inputs, 'source_sha256': source_hashes, 'replays': rows,
            'main_alu_instruction_means': means, 'main_alu_instruction_change_percent': changes,
            'limitations': [
                ...
                'Candidate reverted: small net arithmetic saving shifts work into the expensive vertex stage; no speedup or energy/thermal benefit established.',
