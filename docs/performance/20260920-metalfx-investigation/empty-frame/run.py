...
try:
    ...
    with (root / (case + '.log')).open('w') as log, (root / (case + '.power.jsonl')).open('w') as telemetry:
        ...
        command = [str(root / 'YarraEmptyFrame.app/Contents/MacOS/empty_frame'), *args]
...
if start is not None and end is not None:
    ...
    result.update(start=lines[start], complete=lines[end], packets=len(packets), hud_gpu_median=stats.median(gpu) if gpu else None, hud_interval_mean=stats.mean(interval) if interval else None, power_samples=len(samples))
