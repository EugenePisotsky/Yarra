...
try:
    ...
    with (root / (case + '.log')).open('w') as log, (root / (case + '.power.jsonl')).open('w') as telemetry:
        power = subprocess.Popen(['tmp/terrain-gpu-headroom/macmon', 'pipe', '--samples', '40'], stdout=telemetry, stderr=subprocess.DEVNULL)
        command = ['tmp/YarraReleaseProfile.app/Contents/MacOS/yarra-app-game', '--world-db', 'tmp/cloud-performance/static.sqlite', '--start-view', 'tmp/cloud-performance/close-view.ron', '--profile-seconds', '10', '--profile-warmup', '15', '--profile-surface', '3456x1942', '--profile-fps', '120', '--profile-grass', 'full', '--gpu-timing-off', '--render-audit', *args]
        game = subprocess.Popen(command, env=env, stdout=log, stderr=subprocess.STDOUT)
        try:
            code = game.wait(timeout=40)
...
result = dict(config=next((s for s in lines if 'GRASS_PROFILE event=config' in s), None), case=case, code=code, command=command, errors=[s for s in lines if 'ERROR' in s or 'panicked' in s])
if start is not None and end is not None:
    ...
    native = [float(re.search(r'gpu_ms=([0-9.]+)', s)[1]) for s in lines[start+1:end] if 'METALFX_TIMING' in s and 'gpu_ms=' in s]
    result.update(native_gpu_median=stats.median(native) if native else None, native_samples=len(native), native_reset_samples=sum('reset=true' in s for s in lines[start+1:end] if 'METALFX_TIMING' in s), valid_focus=('focused=true' in lines[start] and all('focused=true' in s for s in lines[start+1:end] if 'GRASS_PROFILE event=sample' in s)), start=lines[start], complete=lines[end], packets=len(packets), hud_gpu_median=stats.median(gpu) if gpu else None, hud_interval_mean=stats.mean(interval) if interval else None, power_samples=len(samples))
