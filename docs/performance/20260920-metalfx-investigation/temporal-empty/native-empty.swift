...
let color = texture(.rgba16Float, scaler.colorTextureUsage.union(.shaderWrite))
let depth = texture(.depth32Float, scaler.depthTextureUsage)
let motion = texture(.rg16Float, scaler.motionTextureUsage)
let output = texture(.rgba16Float, scaler.outputTextureUsage)
...
emit(["event": "setup", "device": device.name, "os": ProcessInfo.processInfo.operatingSystemVersionString,
