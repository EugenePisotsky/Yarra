...
if start is not None and end is not None:
    ...
    result.update(native_gpu_median=stats.median(native) if native else None, native_samples=len(native), native_reset_samples=sum('reset=true' in s for s in lines[start+1:end] if 'METALFX_TIMING' in s), valid_focus=('focused=true' in lines[start] and 'focused=true' in lines[end]), start=lines[start], complete=lines[end], packets=len(packets), hud_gpu_median=stats.median(gpu) if gpu else None, hud_interval_mean=stats.mean(interval) if interval else None, power_samples=len(samples))
